...
class BucketingSampler(Sampler):
    ...
    def shuffle(self, epoch):
        g = torch.Generator()
        g.manual_seed(epoch)
        rand_ids = list(torch.randperm(len(self.ids), generator=g))
        self.bins = [rand_ids[i:i + self.batch_size] for i in range(0, len(rand_ids), self.batch_size)] 
...
class DistributedBucketingSampler(Sampler):
    ...
    def __iter__(self):
        offset = self.rank
        # add extra samples to make it evenly divisibl
